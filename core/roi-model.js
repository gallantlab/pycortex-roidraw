 ...
 *
 * An ROI is { id, name, color, left:[subjectIdx], right:[subjectIdx], outline:[{h,g}], labelVert:{h,g} }.
 * The serialized form references SUBJECT vertex indices, so it ports to any viewer built on the
 * same surface. `outline`/`labelVert` are how the boundary + label are reconstructed.
 */

export const FORMAT = "pycortex-roidraw/vertexset-v1";
...
export class ROISet {
    ...
    add({ name, color, left, right, outline = null, labelVert = null }) {
        const roi = {
            id: this.nextId++,
            name: name,
    ...
    toJSON(surfaceId) {
        return {
            ...
            note: "Per-hemisphere subject vertex indices + an ordered boundary ring (outline). " +
                  "Portable to any viewer built on the same surface.",
